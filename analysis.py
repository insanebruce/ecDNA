...
ref = pd.read_csv("pcaa-master-platinum_set_2020.tsv", sep="\t", header=0, usecols=[0,7,8,12,13,28,32])

# Only keep TCGA samples
ref = ref[ref['study'] == 'TCGA']

# Load the oncoprint matrix after querying genes against all TCGA studies
res = pd.read_csv("PATIENT_DATA_oncoprint.tsv", sep="\t", header=0, skiprows=[1,2,3,4])
...
df1 = res[cols_met1]
df1.to_csv("oncoprint_1.tsv", sep="\t", index=False)
...
df2 = res[cols_met1]
df2.to_csv("oncoprint_2.tsv", sep="\t", index=False)